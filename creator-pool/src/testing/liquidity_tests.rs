...
use cosmwasm_std::{
    ...
    to_json_binary, Addr, BankMsg, Binary, Coin, ContractResult, CosmosMsg, Decimal, OwnedDeps,
    SystemError, SystemResult, Timestamp, Uint128, WasmMsg, WasmQuery,
};
...
use crate::state::{
    PoolDetails, Position, IS_THRESHOLD_HIT, NEXT_POSITION_ID, POOL_FEE_STATE, POOL_STATE,
...
fn test_deposit_liquidity_imbalanced_amounts() {
    ...
    let refund_msg = res
        .messages
        .iter()
        .find(|msg| matches!(&msg.msg, CosmosMsg::Bank(BankMsg::Send { .. })));
    assert!(refund_msg.is_some());

    let refund_attr = res
        .attributes
        .iter()
        .find(|a| a.key == "refunded_amount0")
        .unwrap();
...
fn test_remove_partial_liquidity_by_percent() {
    ...
    assert_eq!(
        res.attributes
            .iter()
            .find(|a| a.key == "liquidity_removed")
            .unwrap()
            .value,
        "250000" // 25% of 1M
    );
...
fn test_add_to_position_collects_fees_first() {
    ...
    assert_ne!(
        fees_collected, "0",
        "Fees should have been collected during add_to_position"
    );
...
pub fn create_test_position(
    ...
    LIQUIDITY_POSITIONS
        .save(&mut deps.storage, &position_id.to_string(), &position)
        .unwrap();

    POOL_STATE
        .update(
            &mut deps.storage,
            |mut state| -> Result<_, cosmwasm_std::StdError> {
...
fn test_dust_positions_get_heavily_penalized() {
    let dust_positions = vec![1u128, 10u128, 100u128, 1000u128];
...
fn test_fee_distribution_proportional() {
        ...
        match query {
            WasmQuery::Smart { contract_addr, msg } => {
                if contract_addr == "nft_contract" {
                    ...
                    if let Ok(Cw721QueryMsg::OwnerOf { token_id, .. }) =
                        from_json::<Cw721QueryMsg>(msg)
                    {
    ...
    assert!(
        ratio > 1.9 && ratio < 2.1,
        "Fee ratio {} should be close to 2.0",
        ratio
    );
}
...
fn test_deposit_underpayment_overflow() {
    ...
    match res {
        Ok(r) => {
            ...
            assert_eq!(
                amount0, "10",
                "Should limit usage to available bluechip (10)"
            );
        }
        Err(e) => {
