...
/// Helper to set up multiple positions at once
pub fn setup_multiple_positions(
    deps: &mut OwnedDeps<MockStorage, MockApi, MockQuerier>,
    positions: Vec<(u64, &str, Uint128)>, // (id, owner, liquidity)
) {
    for (id, owner, liquidity) in positions {
        create_test_position(deps, id, owner, liquidity);
    }
}
