...
bluechipChaind tx wasm execute $ORACLE_ADDR \
  '{"set_price":{"price_id":"ATOM_USD","price":"100000000000"}}' \
  --from $FROM \
  --chain-id $CHAIN_ID \
  --keyring-backend $KEYRING \
  -y
...
# Step 9: Attempt Swap
echo ""
echo "🔄 Attempting Swap (Should fail if threshold not crossed)..."
...
SWAP_TX=$(bluechipChaind tx wasm execute $POOL_ADDR "$SWAP_MSG"   --amount "${SWAP_AMOUNT}stake"   --from $FROM   --chain-id $CHAIN_ID   --gas auto --gas-adjustment 1.3   --keyring-backend $KEYRING   -y --output json)

echo "Swap TX Response:"
echo $SWAP_TX | jq .

# Check for error in raw log
RAW_LOG=$(echo $SWAP_TX | jq -r '.raw_log')
if [[ "$RAW_LOG" == *"You can not swap until the threshold is crossed"* ]]; then
  echo "❌ REPRODUCTION SUCCESSFUL: Swap failed with expected error."
else
  echo "❓ Swap result unexpected: $RAW_LOG"
fi
